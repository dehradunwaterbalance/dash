...
app.title = 'CURE India USAID Dehradun Water Balance Portal'


# ## Setup Paths to External Files and Initial JSON Setup

with open('/home/manaruchi/mysite/main.geojson') as f:
...
df = pd.read_csv('/home/manaruchi/mysite/values.csv')

df['GRID_ID'] = df['OID_'].values

df = df.round(3)

ground_water_df = pd.read_csv(r'/home/manaruchi/mysite/Maps/ground_water_level/values.csv').round(3)
other_details_df = pd.read_csv(r'/home/manaruchi/mysite/Maps/zones.csv').round(3)
other_details_df = other_details_df[other_details_df['Litho']>0]
lulc_df = other_details_df[other_details_df['LULC_Code']!=0]


with open(r'/home/manaruchi/mysite/jalsansthan_zones_for_dash.geojson') as f:
    zones_json = json.load(f)
zones_df = pd.read_csv('/home/manaruchi/mysite/Maps/zones_jalsansthan/Jalsansthan_Zone.csv')

stream_json = dict()

with open(r'/home/manaruchi/mysite/streams/1990.geojson') as f:
    stream_json['1990'] = json.load(f)
with open(r'/home/manaruchi/mysite/streams/1994.geojson') as f:
    stream_json['1994'] = json.load(f)
with open(r'/home/manaruchi/mysite/streams/1999.geojson') as f:
    stream_json['1999'] = json.load(f)
with open(r'/home/manaruchi/mysite/streams/2004.geojson') as f:
    stream_json['2004'] = json.load(f)
with open(r'/home/manaruchi/mysite/streams/2009.geojson') as f:
    stream_json['2009'] = json.load(f)
with open(r'/home/manaruchi/mysite/streams/2014.geojson') as f:
    stream_json['2014'] = json.load(f)
with open(r'/home/manaruchi/mysite/streams/2019.geojson') as f:
        ...
        dbc.CardBody(
            [
                ...
                html.Div([dcc.Loading(id = 'maparea', type = 'dot'),
                    html.Div(overlay_contents,id = 'overlay', style = {'top': '85%', 'left': '17%', 'z-index': 2,'position': 'fixed', 'padding': 10, 'border-radius': 0, 'border': '2px solid #ffffff', 'width': 200, 'background-color': '#ffffff', 'opacity': 0.8, 'align-content': 'center'})], style = {'padding': '0px'})
...
navbar = dbc.Navbar(
    [
        dbc.Col(html.Div("CURE India USAID Dehradun Water Balance Portal", style = {'color': 'white', 'font-size': 'large'})),
        ...
        html.A(
            ...
            dbc.Row(
                [
                    dbc.Col(html.Img(id = 'curelogo', src=cure_logo, height="50px", alt = 'Centre for Urban and Regional Excellence')),
        ...
        html.A(
            ...
            dbc.Row(
                [
                    dbc.Col(html.Img(id = 'usaidlogo', src=usaid_logo, height="50px", alt = 'U.S. Agency for International Development')),
...
row = dbc.Row(
    [
        dbc.Col(html.Div(first_column, id = 'first-col', style = {'height': '700px', 'overflow': 'auto'}), width = 2),
        dbc.Col(html.Div(second_column, style = {'height': '700px'}), width = 7),
        dbc.Col(html.Div(third_column, style = {'height': '700px'}), width = 3),
    ],
    no_gutters=True,
    style = {'height': '600px'}
)
...
@app.callback(Output('year-select', 'options'),
             [Input('button-item8', 'active'), Input('button-item2', 'active'), Input('button-item', 'active'), Input('button-item7', 'active'), Input('button-item5', 'active'), Input('button-item3', 'active')])
def select_ground_water16(n1,n2,n3,n4,n5,n6):
    if(n1):
...
@app.callback(Output('year-select', 'value'),
             [Input('button-item8', 'active'), Input('button-item2', 'active'), Input('button-item', 'active'), Input('button-item7', 'active'), Input('button-item5', 'active'), Input('button-item3', 'active')])
def select_ground_water17(n1,n2,n3,n4,n5,n6):
    if(n1):
        return "jan11"
    if(n2):
        return '2001'
...
def select_ground_water19(selected_val, activeornot, n2,  n4, n5, n6, n7, zonebut):
    if(activeornot):
        ...
        return {'top': '650px', 'left': '17%', 'z-index': 2,'position': 'fixed', 'padding': 10, 'border-radius': 0, 'border': '2px solid #ffffff', 'width': 200, 'background-color': '#ffffff', 'opacity': 0.8, 'align-content': 'center'}

    if(n2):

        return {'top': '653px', 'left': '17%', 'z-index': 2,'position': 'fixed', 'padding': 10, 'border-radius': 0, 'border': '2px solid #ffffff', 'width': 200, 'background-color': '#ffffff', 'opacity': 0.8, 'align-content': 'center'}
    ...
    if(n4):
        ...
        if(selected_val == 'LULC_Code'):
            ...
            return {'top': '592px', 'left': '17%', 'z-index': 2,'position': 'fixed', 'padding': 10, 'border-radius': 0, 'border': '2px solid #ffffff', 'width': 200, 'background-color': '#ffffff', 'opacity': 0.8, 'align-content': 'center'}


        elif(selected_val == 'Litho'):

            return {'top': '550px', 'left': '17%', 'z-index': 2,'position': 'fixed', 'padding': 10, 'border-radius': 0, 'border': '2px solid #ffffff', 'width': 450, 'background-color': '#ffffff', 'opacity': 0.8, 'align-content': 'center'}
    ...
    if(zonebut):
        ...
        return {'top': '550px', 'left': '17%', 'z-index': 2,'position': 'fixed', 'padding': 10, 'border-radius': 0, 'border': '2px solid #ffffff', 'width': 230, 'background-color': '#ffffff', 'opacity': 0.8, 'align-content': 'center'}
